...
from .models import Car, Order, Profile
...
def index(request):
    ...
    return render(request, 'shop/index.html', {'cars': cars})
...
def order_confirmation(request, order_id):
    try:
        ...
        total_price = sum(order.car.price * order.quantity for order in orders)  # Calculate total price
        if request.method == 'POST':
    ...
    order_totals = []
    for order in orders:
        order_total = order.car.price * order.quantity
        order_totals.append({
            'order': order,
